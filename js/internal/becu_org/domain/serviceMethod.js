...
//                        for (n in model) {
//                            //props can be functions, so don't rule them out
//                            if (n == requiredField && (ko.unwrap(model[n]) != undefined)) {
//                                found = true;
//                                break;
//                            }
//                        }
...
//                        //if field not found in the non-required fields collection, and the field does not hold a value, then error condition
//                        if (!found && (ko.unwrap(model[nonfield]) == undefined)) {
//                            //return false;
//                            if (this._inMissingFields(nonField) == -1) {
//                                this.missingFields.push({ field: nonField, description: '' });
//                            }
...
//            }
    ...
    becu_org.communication.callSpec = (function () {
        ...
        return new JS.Class('becu_org.communication.callSpec', {
        ...
        ,
            isReady: function () {
                ...
                for (var i = 0; i < self.need().length; i++) {
                    var type = self.need()[i].type;
                    var found = false;

                    for (var j = 0; j < self.have().length; j++) {
                        var instance = self.have()[j];
    ...
    becu_org.domain.serviceMethod = (function () {
        ...
        return new JS.Class({
            ...
            initialize: function (config) {
                ...
                this.resultViewUri = '/views/customer.htm?r=2'; // = resultViewUri;
                this.resultViewModelName = 'littleUmbrella.circleverse.viewModel.CustomerViewModel'; // = resultViewUri;
                ...
                this.opts = {flushWhenFinished: true};
                // By default templates are pulled from the same server path as the html file making the request.
                // If you keep your templates in a separate directory, specify it here as a relative path to the html document making the request
                // For example, /Templates for a sub dir, or ../Templates for going up a dir level and dropping into a Templates folder
                //this.templateUrl = "";

                var sub = this.callSpec.subscribe(function(val){

                    val.subscribe(self.methodRequirementMet, self);
                });
        ...
        ,
            ...
            methodRequirementMet: function(message){
                var self = this;

                if (this.callSpec() && this.callSpec().isReady()){
                    //todo: get real
                    //stub for now
                    this.call();
                    var cust = new becu_org.domain.model.CustomerObservable(), a, c, info, tran, r, role, aalert, s, t, prod, limit = 5;
                   // c = cust;
                    
                    cust.firstName("Ms. BECU");
                    cust.lastName("User");
                    ...
                    c.firstName("Ms. Member");
                    c.lastName("#" + j);
                                

                            if (i < 2){
                                role = new becu_org.domain.model.AccountRoleObservable();
...
//                                    a.relationships.push(r);
                            ...
                            aalert = new becu_org.domain.model.AlertScheduleBalanceObservable();                            
                            a.alerts.push(aalert);
                            
                            s = new becu_org.domain.model.PaymentScheduleObservable();                            
                            a.paymentSchedules.push(s);
                            
                            tran = new becu_org.domain.model.AccountTransactionObservable();                            
                            a.transactions.push(tran);    
                        }
                    ...
                    JS.require(self.resultViewModelName, function(){
                        //eaf.util.namespace returns an object
                        var type = eaf.util.namespace(self.resultViewModelName);
                        var vm = new type(cust, self.resultViewUri);
