	...
	handler: function( event ){ 
		...
		switch ( event.type ){
			...
			case 'mouseup': 
				...
				if ( dd.dragging ){
				    if ( dd.drop !== false && $special.drop ){
                        //as of jquery 1.7.1 (or before?) this can prevent/suppress the "dragend" event from firing
                        //var oldIsPropagationStopped = event.isPropagationStopped;
						$special.drop.handler( event, dd ); // "drop"
                        //event.isPropagationStopped = oldIsPropagationStopped;
                    }	
