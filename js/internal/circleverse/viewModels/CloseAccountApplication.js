...
circleverse.viewModel.CloseAccountApplication = (function () {
    ...
    return new JS.Class("circleverse.viewModel.CloseAccountApplication", {



        initialize: function (eventAggregator, customerService, accountService, sidePanelSummaryViewModel, sidePanelFormsViewModel, sidePanelHelpViewModel, closeAccountViewModel, requestInterceptorCloseAccount, spRemote) {// (tracker, uri, templateUri, templateId, resultTemplateUri, callSpec, name, id, businessClass, opts) {
            //properties
            //this.callSuper(text);
            ...
            this.sidePanelSummaryViewModel = sidePanelSummaryViewModel;
            this.closeAccountViewModel = closeAccountViewModel;

            self.session = new becu_org.UserSession();
            ...
            var taxId, accountNumber;

            try {
                requestInterceptorCloseAccount.init();
                taxId = requestInterceptorCloseAccount.taxId;
                accountNumber = requestInterceptorCloseAccount.accountNumber;
            }
            catch (e) {
                this.hasErrors(true);
                this.errors.push({ message: 'unable to find detect tax id or account #' });
                //return;
            }

            if (null == taxId || null == accountNumber) {
                this.hasErrors(true);
                this.errors.push({ message: 'tax id or account # was not passed' });
                //return;
            }


            this.sidePanelSummaryViewModel.accountInformationViewModel().isReady(false);
            this.sidePanelSummaryViewModel.stepBasicViewModel().isReady(false);

            var cust = this.customerService.getCustomer(taxId);
            setTimeout(function () {
                self.sidePanelSummaryViewModel.stepBasicViewModel().isReady(true);
                self.eventAggregator.publish('circleverse.viewModel.CloseAccountApplication.Customer.Changed', cust);

            }, 3000);

            var account = this.accountService.getAccount(accountNumber);
            setTimeout(function () {
                self.sidePanelSummaryViewModel.accountInformationViewModel().isReady(true);
                self.eventAggregator.publish('circleverse.viewModel.CloseAccountApplication.AccountToClose.Changed', account);

            }, 3000);

            this.pmms = ko.observableArray();

            this.accordionVm = new circleverse.controls.viewModel.AccordionViewModel([
            { key: 'Help', expanded: false }
            , { key: 'Forms', expanded: false }
            , { key: 'Summary', expanded: true, vm: this.sidePanelSummaryViewModel}]);
